...
export const tableConfig = () => ({
  title: "Raw Data",
  colDef: ["timestamp", "impressions", "clicks", "cost", "conversions"],
});

export const impressionsModel = (state) =>
  dataModel(state).map(({ impressions, timestamp }) => ({
    impressions,
    date: moment(timestamp).format("DD/MM"),
  }));

export const impressionsConfig = () => ({
  title: "Website Impressions",
  valueField: "impressions",
  argumentField: "date",
});

export const costPerClickModel = (state) =>
  dataModel(state).map((d) => ({
